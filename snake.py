...
def move():
    ...
    if can.coords(teteSerpent)[0]<0 or can.coords(teteSerpent)[1]<0 or can.coords(teteSerpent)[0]>450 or can.coords(teteSerpent)[1]>450:
        end()
        return
    ...
    for i in listeCompteur:
        nom = can.create_oval(registre[i][0],registre[i][1], registre[i][0]+50, registre[i][1]+50, fill="#70F047",width=2)
...
bouttonLancer = Button(fen, text='Lancer', command = lambda:[start(),move()], bg='black' , fg='green',width=30,height=15)
